...
function sys=mdlDerivatives(t,x,u,P)
  ...
  phi_ff = 0; % feedforward roll command
  ...
  % solve for heading 
  psi = chi - asin((1/Va) * (-wn*sin(chi)+we*cos(chi)));

  % compute groundspeed
  pndot = Va*cos(psi) + wn;
  pedot = Va*sin(psi) + we;

  % don't let turn rate exceed g/V*tan(phi_max)
  chiddot = P.b_chidot*(chi_c_dot-chidot) + P.b_chi*(chi_c-chi);
  if (chidot>=P.gravity/Va*tan(P.phi_max)) & (chiddot>0),
      chiddot = 0;
  elseif (chidot<=-P.gravity/Va*tan(P.phi_max)) & (chiddot<0),
      chiddot = 0;
  end

  % don't let climb rate exceed Va*sin(\gamma_max)
  hddot = P.b_hdot*(h_c_dot-hdot) + P.b_h*(h_c-h);
